...
function finishDrawPath()
{
    ...
    {
       finishedElem.setAttribute("fill", "white")
        finishedElem.setAttribute("fill-opacity", 0)
    }
    else
    {
      ...
      if(cw.drawPathRightAngleCheck.checked==true)
    {
        ...
        PathLLArray =[];
        for(var k = 0; k<segs-1; k++)
        {
            var mySeg = pathSegs.getItem(k)

            var latLng = PlanetProjection.invert([mySeg.x, mySeg.y])
    ...
    {
       finishedElem.setAttribute("rightAngle", "false")
        if(linearD.indexOf("Z")!=-1)
        {

            PathLLArray.push(PathLLArray[0])
        ...
        finishedElem.setAttribute("myZoom", PlanetView.k)
             console.log("bogie - for date")
                ...
                finishedElem.setAttribute("ondblclick", "startPathDrawEdit("+id+")")

                if(cw.drawPathCommentValue.value!="Path comment goes here(optional)..." && cw.drawPathCommentValue.value!="")
        {
                finishedElem.setAttribute("comment", txt2xml(cw.drawPathCommentValue.value))
        ...
        if(oEMAIL)
                finishedElem.setAttribute("createdBy", oEMAIL)
                 ...
                 if(cw.drawPathStrokeArrowCheck.checked==true&&NewEndArrow==true)
                {

                        sendArrowDef(EndArrowId)

                }


    finishedElem.setAttribute("pointsLL", PathLLArray.toString())
    ...
    var bbCx = bbx+.5*bbw
    var bbCy = bby+.5*bbh
    var pathBBCoords = PlanetProjection.invert([bbCx, bbCy])

    AddPathBBCoordsArray.push([pathBBCoords, ActiveScale, id])
    finishedElem.setAttribute("bbCoords", pathBBCoords)
