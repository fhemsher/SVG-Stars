 ...
 function updatePosts(doc,folder)
{
 ...
 var ownersXML =Server.CreateObject("Msxml2.DOMDocument.6.0");
   ...
   var timelineLastValue
    if(sendObj.getAttribute("timelineLastValue"))
        timelineLastValue=parseInt(sendObj.getAttribute("timelineLastValue"),10)
     if(timelineLastValue)
    {
       var timelineXML = Server.CreateObject("Msxml2.DOMDocument.6.0");
    	var xmlFile='../../Communities/'+folder+'/Timeline.xml'
    	var timelineMap=Server.MapPath(xmlFile)
    	timelineXML.load(timelineMap)
        var timelineDoc=timelineXML.documentElement
        var lastValue=parseInt(timelineDoc.getAttribute("timelineLastValue"),10)
        if(timelineLastValue>lastValue)
        {
            timelineDoc.setAttribute("timelineLastValue",timelineLastValue)
            if(sendObj.getAttribute("timelineLastDateString"))
                timelineDoc.setAttribute("timelineLastDateString",sendObj.getAttribute("timelineLastDateString"))

            timelineXML.save(timelineMap)
        }
    }
