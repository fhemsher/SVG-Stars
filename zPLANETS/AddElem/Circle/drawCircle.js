...
function closeDrawCircle()
{
    if(addElemCircleViz==true)
    {
        ...
        if(document.getElementById("activeElem"))
        {
            ...
            planetG.appendChild(dragDot) //--place drag dot on top---
...
function startCircleDraw()
{
    ...
    if(EditCircle==false)
    {
        ...
        if(LatLngPntSet==true)
            ...
            {
                planetSVG.setAttribute('onclick', "placeDrawCircle()") //---click to add more icons for this session---
...
function placeDrawCircle()
{
    ...
    ActiveElem.attr("r", 50)

    DragDot.style("visibility", "visible")

    planetSVG.removeAttribute('onclick')
    planetSVG.style.cursor = ""
    planetSVG.setAttribute("onmousedown", "startDragCircle(evt)")
    planetSVG.setAttribute("onmousemove", "dragCircle(evt)") //;showPlanetComment(evt)
    planetSVG.setAttribute("onmouseup", "endDragCircle(evt)")
    ...
    if(EditThisCircle.getAttribute("comment"))
        ...
        else
        {
            ...
            cw.drawCircleStrokeDashCheck.checked = true
                ...
                //---timeout??---
                planetSVG.setAttribute("onmousedown", "startDragCircle(evt)")
                planetSVG.setAttribute("onmousemove", "dragCircle(evt)")
                planetSVG.setAttribute("onmouseup", "endDragCircle(evt)")
        ...
        finishedElem.setAttribute("id", DrawCircleEditId)
                ...
                for(var k = 0; k<AddElemCoordsArray.length; k++)
            {
                ...
                if(addId==DrawCircleEditId)
                {
                    ...
                    AddElemCoordsArray[k] =[ll, ActiveScale, DrawCircleEditId, '']
...
function resetEditCircle()
{
    ...
    ActiveElem = null
    activeElem = null
    planetViewG.appendChild(dragDot) //--place drag dot on top---
    DrawX.style("display", "none")
    ...
    cw.drawCircleFinishButton.disabled = false
    DrawCircle = true
    planetSVG.setAttribute('onclick', "placeDrawCircle()")
