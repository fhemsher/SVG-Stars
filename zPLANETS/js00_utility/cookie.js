...
function deleteAllPlanetCookies() {
    var cookies = document.cookie.split(";");

    for (var i = 0; i < cookies.length; i++) {
    	var cookie = cookies[i];
        if(cookie.indexOf("_SVGPlanets_")!=-1)
        {
...
function setExitCookie()
{
      ...
      {     var lastLoginMS= mostRecentCookie()
        ...
        {     var cookieFolder = getCookie("_SVGPlanets_Folder@"+lastLoginMS)
            setCookie(cookieFolder+"_SVGPlanets_DrawingName@"+lastLoginMS, "", -1)
            setCookie(cookieFolder+"_SVGPlanets_PlaceDescription@"+lastLoginMS, "", -1)
            setCookie(cookieFolder+"_SVGPlanets_ID@"+lastLoginMS, "", -1)
            setCookie(cookieFolder+"_SVGPlanets_Owner_NAME@"+lastLoginMS, "", -1)
            setCookie(cookieFolder+"_SVGPlanets_Owner_EMAIL@"+lastLoginMS, "", -1)
            setCookie(cookieFolder+"_SVGPlanets_User_EMAIL@"+lastLoginMS, "", -1)
            setCookie("_SVGPlanets_Folder@"+lastLoginMS, "", -1)
        }
...
*/
function removeFolderPlanetCookies()
{
...
pCOOKIES = new Array();
	...
	for(bb = 0; bb < pCOOKIES.length; bb++)
	{
		...
		{  var utcms=NmeVal[0].split("@")[1]
           var folderCookie="_SVGPlanets_Folder@"+utcms
           if(folderCookie==NmeVal[0])
		   {
              	var folder=NmeVal[1]
					...
					for(dd = 0; dd < dCOOKIES.length; dd++)
					{
						...
						if(dNmeVal[0])
						{
                           setCookie(folder+"_SVGPlanets_PlaceNAME@"+utcms,"",-1)
                           setCookie(folder+"_SVGPlanets_PlaceDescription@"+utcms,"",-1)
                           setCookie(folder+"_SVGPlanets_ID@"+utcms,"",-1)
		                 	setCookie(folder+"_SVGPlanets_Owner_NAME@"+utcms,"",-1)
							setCookie(folder+"_SVGPlanets_Owner_EMAIL@"+utcms,"",-1)
							setCookie(folder+"_SVGPlanets_User_EMAIL@"+utcms,"",-1)
							setCookie("_SVGPlanets_Folder@"+utcms,"",-1)
						}
					}
			}
		}
...
function removeMobileCookies()
{
	...
	for(bb = 0; bb < pCOOKIES.length; bb++)
	{
		...
		{  var utcms=NmeVal[0].split("@")[1]
           ...
           var folderCookie="Mobile_SVGPlanets_Folder@"+utcms
           if(folderCookie==NmeVal[0])
		   {
              	var folder=NmeVal[1]
                  	dCOOKIES = new Array();
					...
					for(dd = 0; dd < dCOOKIES.length; dd++)
					{
						...
						if(dNmeVal[0])
						{
						    setCookie(folder+"_Mobile_SVGPlanets_Owner_NAME@"+utcms,"",-1)
						     setCookie(folder+"_Mobile_SVGPlanets_Owner_EMAIL@"+utcms,"",-1)
                              setCookie(folder+"_Mobile_SVGPlanets_Place_ID@"+utcms,"",-1)
                              setCookie(folder+"_Mobile_SVGPlanets_PlaceNAME@"+utcms,"",-1)
                              setCookie(folder+"_Mobile_SVGPlanets_User_EMAIL@"+utcms,"",-1)

                              setCookie("Mobile_SVGPlanets_Folder@"+utcms,"",-1)
