...
function closePublishHelp()
{
  ...
  function openRaDecHelp()
{
    closeCurrentHelp()
    CurrentHelpDiv='raDecHelpDiv'
     raDecHelpDiv.style.display = "block"
    var height = raDecHelpDiv.scrollHeight
    d3.select("#raDecHelpDiv").transition().duration(800).style("height", height+"px")


}

function closeRaDecHelp()
{
    var height = 1
    d3.select("#raDecHelpDiv").transition().duration(800).style("height", height+"px")
    setTimeout('raDecHelpDiv.style.display="none"', 900)
        CurrentHelpDiv=null
}

  function openMeasureHelp()
{
    closeCurrentHelp()
    CurrentHelpDiv='measureHelpDiv'
     measureHelpDiv.style.display = "block"
    var height = measureHelpDiv.scrollHeight
    d3.select("#measureHelpDiv").transition().duration(800).style("height", height+"px")


}

function closeMeasureHelp()
{
    var height = 1
    d3.select("#measureHelpDiv").transition().duration(800).style("height", height+"px")
    setTimeout('measureHelpDiv.style.display="none"', 900)
        CurrentHelpDiv=null
}

  function openSymbolHelp()
{
    closeCurrentHelp()
    CurrentHelpDiv='symbolHelpDiv'
     symbolHelpDiv.style.display = "block"
    var height = symbolHelpDiv.scrollHeight
    d3.select("#symbolHelpDiv").transition().duration(800).style("height", height+"px")


}

function closeSymbolHelp()
{
    var height = 1
    d3.select("#symbolHelpDiv").transition().duration(800).style("height", height+"px")
    setTimeout('symbolHelpDiv.style.display="none"', 900)
        CurrentHelpDiv=null
}
