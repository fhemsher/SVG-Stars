  ...
  var zoomPoint,
      ...
      zoom = d3.behavior.zoom()
        .on("zoomstart", function() {
             if(StopPlanetZoom==false&& dblClickError==false)
            {
                  ...
                  zoomOn.call(zoom, "zoom", function() {
                        ...
                        zoomed(event.of(this, arguments));
                         rotationValue.value= view.r[0].toFixed(2)+", "+view.r[1].toFixed(2)+", "+view.r[2].toFixed(2)
                      });
                  zoomstarted(event.of(this, arguments));
            }
           dblClickError=false    //---reset---
         })
        .on("zoomend", function() {
    ...
    g.each(function() {
      ...
      if (transition !== g  ) {
        transition
           ...
           if(dblClickError==false)
           {
                transition.tween("zoom:zoom", function() {
                var width = zoom.size()[0],
                i = interpolateBetween(quaternionFromEuler(View.r), quaternionFromEuler(View.r));

                var error2=false
                try
                {
                    d3.geo.distance(View.r, View.r)
                }
                catch(e)
                {
                    error2=true
                ...
                if(error2==false)
                {
                    d = d3.geo.distance(View.r, View.r);
                    smooth = d3.interpolateZoom([0, 0, width / View.k], [d, 0, width / View.k]);
