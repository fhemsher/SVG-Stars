...
function startDragPoint(evt)
{
	if(!Dragging) //---prevents dragging conflicts on other draggable elements---
	{
		...
		var Pnt = pnt.matrixTransform(sCTM.inverse());
  			...
  			TransformRequestObj = planetSVG.createSVGTransform()
		...
		if(cw.drawPathRightAngleCheck.checked==true)
		{
			 ...
			 var circles=dragCircleG.childNodes //---includes DrawX---
				...
				if(trackX.toFixed(8)==PreX.toFixed(8)){PreX="track"}
				if(trackX.toFixed(8)==PostX.toFixed(8)){PostX="track"}
				if(trackY.toFixed(8)==PreY.toFixed(8)){PreY="track"}
				if(trackY.toFixed(8)==PostY.toFixed(8)){PostY="track"}
		   }
        ...
        //commentDiv.style.left=SVGx+10+"px"
			...
			SegNextY=NextY
           	...
           	PathPointArray[DragPointNum]=[NextX,NextY]
            	PathLLArray[DragPointNum]=LatLng
           ActiveElemStop=false
            ...
            // DrawX.attr("transform",StarPoint(ActivePoint))
             //DragCircleG.attr("transform",StarPoint(ActivePoint))
                ...
                var pnt=PathLLArray[DragPointNum]
      ...
      commentDiv.style.visibility="hidden"
       ...
       //--build new array needed for tracktable if right angle---
        ...
        /*
		  ...
		  //}
       */
      if(DrawPathType=="basis")StopPlanetZoom=true
    }
}
...
if(cw.drawPathRightAngleCheck.checked==true)
{
	 ...
	 else  //---last seg is a Z----
	 {
		...
		if(trackX==PreX){PreX="track"}
		if(trackX==PostX){PostX="track"}
		if(trackY==PreY){PreY="track"}
		if(trackY==PostY){PostY="track"}
   }
}
