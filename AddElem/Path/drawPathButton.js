...
function closeDrawPath()
{
    ...
    if(document.getElementById("drawPathSmooth"))
        ...
        starSVG.removeEventListener("keypress", keyPressPath)
...
function finishDrawPath()
{
      ...
      if(cw.drawPathRightAngleCheck.checked==true)
    {
        ...
        for(var k = 0; k<segs-1; k++)
        {
            ...
            var latLng = StarProjection.invert([mySeg.x, mySeg.y])
       ...
       finishedElem.setAttribute("rightAngle", "false")
        ...
        if(oEMAIL)
                finishedElem.setAttribute("createdBy", oEMAIL)
                        ...
                        if(cw.drawPathFadeCheck.checked==true)
    ...
    var bbCx = bbx+.5*bbw
    var bbCy = bby+.5*bbh
    var pathBBCoords = StarProjection.invert([bbCx, bbCy])
    var fade = finishedElem.getAttribute("fade")
    AddPathBBCoordsArray.push([pathBBCoords, ActiveScale, id, fade])
