...
function editAddPathElem(elemObjEdit)
{
    ...
    starSVG.setAttribute("onmousedown", "startDragEdit(evt)")
    starSVG.setAttribute("onmousemove", "dragEdit(evt)")
    starSVG.setAttribute("onmouseup", "endDragEdit()")
    var cw = addElemPathEditCw
    ...
    if(fade)
        cw.drawPathEditFadeCheck.checked = true
        else
            cw.drawPathEditFadeCheck.checked = false
...
function setActivePathEdit()
{
  ...
  var t3 = d3.transform(ActiveElem.attr("transform"))
    ...
    domActiveElemG.setAttribute("transform", StarPoint(EditBBCoords)+"scale("+(StarView.k/StarScale)/ActiveScale+")")
    editThisPath()
...
function dragPathChecked()
{
    ...
    {
        starSVG.setAttribute("onmousedown", "startDragPath(evt)")
        starSVG.setAttribute("onmousemove", "dragPath(evt)")
        starSVG.setAttribute("onmouseup", "endDragPath(evt)")
    ...
    else
    {
        starSVG.setAttribute("onmousedown", "startDragEdit(evt)")
        starSVG.setAttribute("onmousemove", "dragEdit(evt)")
        starSVG.setAttribute("onmouseup", "endDragEdit(evt)")
...
function dragDrawPathEditFinish()
{
    if(ActiveElem)
    {
        ...
        for(var k = 0; k<segs; k++)
        {
            ...
            if(segObj.x && segObj.y)
            {
                var starSVGPoint = svgRoot.createSVGPoint();
                starSVGPoint.x = segObj.x
                starSVGPoint.y = segObj.y
                starSVGPointTrans = starSVGPoint.matrixTransform(sCTM)
                segObj.x = starSVGPointTrans.x
                segObj.y = starSVGPointTrans.y
                PathPointArray.push([segObj.x, segObj.y])
            }

            if(segObj.x1 && segObj.y1)
            {
                var starSVGPoint1 = svgRoot.createSVGPoint();
                starSVGPoint1.x = segObj.x1
                starSVGPoint1.y = segObj.y1
                starSVGPointTrans1 = starSVGPoint1.matrixTransform(sCTM)
                segObj.x1 = starSVGPointTrans1.x
                segObj.y1 = starSVGPointTrans1.y
                PathPointArray.push([segObj.x1, segObj.y1])
            }
            if(segObj.x2 && segObj.y2)
            {
                var starSVGPoint2 = svgRoot.createSVGPoint();
                starSVGPoint2.x = segObj.x2
                starSVGPoint2.y = segObj.y2
                starSVGPointTrans2 = starSVGPoint2.matrixTransform(sCTM)
                segObj.x2 = starSVGPointTrans2.x
                segObj.y2 = starSVGPointTrans2.y
