...
function startSymbolDraw()
{
    ...
    starSVG.setAttribute("onclick", "plantSymbol(event)")
...
function plantSymbol(event)
{
    ...
    if(!ActiveSymbol&&cw.SelectedSymbol)
    {
        ...
        DrawSymbol = true
       SymbolPlantStart=false
        var offsets = starDiv.getBoundingClientRect();
        var top = offsets.top;
        ...
        if(LatLngPntSet==false)
        {
            ...
            starSVG.setAttribute("onmousedown", "startDragSymbol(evt)")
            starSVG.setAttribute("onmousemove", "dragSymbol(evt)") //;showStarComment(evt)
            starSVG.setAttribute("onmouseup", "endDragSymbol(evt)")
...
function finishDrawSymbol()
{
    ...
    if(EditSymbol==true)
        ...
        else if(ActiveElem)
        {
            ...
            if(cw.drawSymbolFadeCheck.checked==true)
            ...
            starSVG.setAttribute('onclick', "plantSymbol(event)") //---click to add more icons for this session---
            DrawX.style("display", "none")
...
function setEditSymbol()
{
    ...
    if(EditThisSymbol.getAttribute("comment"))
        ...
        var fade = EditSymbolObj.getAttribute("fade")
        if(fade)
        cw.drawSymbolFadeCheck.checked = true
        else
         ...
         if(EditSymbolObj.getAttribute("filter"))
           ...
           cw.drawSymbolShadowCheck.checked=false
            ...
            //---timeout??---
            starSVG.setAttribute("onmousedown", "startDragSymbol(evt)")
            starSVG.setAttribute("onmousemove", "dragSymbol(evt)")
            starSVG.setAttribute("onmouseup", "endDragSymbol(evt)")
    ...
    if(ActiveElem)
    {
        ...
        if(cw.drawSymbolShadowCheck.checked==true)
                ...
                for(var k = 0; k<AddElemCoordsArray.length; k++)
            {
                ...
                {
                    var fade = finishedElem.getAttribute("fade")
                    AddElemCoordsArray[k] =[ll, ActiveScale, DrawSymbolEditId, '', fade]
...
function resetEditSymbol()
{
    ...
    cw.drawSymbolFinishButton.disabled = false
    DrawSymbol = true
    starSVG.setAttribute('onclick', "placeDrawSymbol()")
